...
class RootElevationTests(unittest.TestCase):
    @contextmanager
    def _patched_environment(self, argv=None, which_paths=None,
                             executable=None):
        ...
        with ExitStack() as stack:
            stack.enter_context(mock.patch.object(
                wifit, 'check_root', return_value=False))
            stack.enter_context(mock.patch.dict(
                wifit.os.environ, {'PREFIX': prefix}))
            stack.enter_context(mock.patch.object(
                wifit.sys, 'executable',
                prefix + '/bin/python3' if executable is None else executable))
            stack.enter_context(mock.patch.object(
                wifit.sys, 'argv', argv or ['wifit.py']))
            stack.enter_context(mock.patch.object(
                wifit, '__file__', prefix + '/bin/wifit.py'))
            stack.enter_context(mock.patch.object(
                wifit.shutil, 'which', side_effect=paths.get))
            stack.enter_context(mock.patch.object(
                wifit.os.path, 'join', side_effect=posixpath.join))
            stack.enter_context(mock.patch.object(
                wifit.os.path, 'isabs', side_effect=posixpath.isabs))
            stack.enter_context(mock.patch.object(
                wifit.os.path, 'isfile', return_value=True))
            stack.enter_context(mock.patch.object(
                wifit.os, 'access', return_value=True))
            stack.enter_context(mock.patch.object(
                wifit.os.path, 'realpath', side_effect=realpath))
    ...
    def test_falls_back_to_sudo_found_on_path_after_launch_error(self):
        ...
        with self._patched_environment(
                ['wifit.py', '--target', 'Cafe WiFi'],
                which_paths={'sudo': fallback_sudo}), \
                mock.patch.object(wifit.os, 'execv', execv):
            with self.assertRaises(ExecReplaced):
                wifit.get_root_access()
    ...
    def test_resolves_relative_python_before_elevation(self):
        ...
        with self._patched_environment(executable='python3'), \
                mock.patch.object(wifit.os, 'execv', execv):
            with self.assertRaises(ExecReplaced):
                wifit.get_root_access()
    ...
    def test_does_nothing_when_already_root(self):
        with mock.patch.object(wifit, 'check_root', return_value=True), \
